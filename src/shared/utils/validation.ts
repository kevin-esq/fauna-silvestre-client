...
export const validateRegisterFields = (fields: RegisterState): string | null => {
  const {
    name, lastName, gender, age, email, password
  } = fields;
  ...
  if (isNaN(parseInt(age)) || parseInt(age) < 18 || parseInt(age) > 65) return 'La edad debe estar entre 18 y 65';

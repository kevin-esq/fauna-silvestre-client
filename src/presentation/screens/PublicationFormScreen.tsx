...
import createPublication from "../../domain/useCases/createPublication";
...
import {useApi} from "../../services/http/ApiProvider";
...
const PublicationFormScreen = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      try {
        publication.img = await ReactNativeBlobUtil.fs.readFile(imageUri, "base64");
      } catch (error) {
        console.error("[PublicationFormScreen] Image read error:", error);
      ...
      await createPublication(
          new PublicationService(
              new ConsoleLogger(),
              httpClient,
              new PublicationRepository(httpClient.instance, new ConsoleLogger())
          ),
          publication,
          "auth.user?.token" // Ensure this comes from actual auth context
      );
      ...
      console.error("[PublicationFormScreen] Submission error:", error);
      Alert.alert("Error", "Ocurrió un error al enviar la publicación");
    }
  };

  const toggleImageExpand = () => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).then(r => console.log(r));
            ...
            <View style={styles.animalOptions}>
              {animalOptions.map((item) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    Haptics.selectionAsync().then(r => console.log(r));
                    setAnimal(item);
                  }}
                  activeOpacity={0.7}>
                  <Text

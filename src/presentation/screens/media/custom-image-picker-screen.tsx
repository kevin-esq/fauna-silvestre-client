      ...
      try {
        ...
        const newPhotos = result.edges.map((edge: PhotoIdentifier) => ({
          ...
          id:
            ...
            `${edge.node.timestamp}-${edge.node.location?.latitude || 0}`
        }));
  ...
  if (error) {
    return (
      <ErrorView
        message={error}
        onRetry={
          selectedAlbum ? () => loadAlbumPhotos(selectedAlbum) : loadAlbums
        }
        styles={styles}
      />
    );
...
const PhotoItem = memo(
  ({ uri, selected, onPress, styles, itemSize }: PhotoItemProps) => (
    <TouchableOpacity
